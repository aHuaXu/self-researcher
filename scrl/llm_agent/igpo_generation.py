...
# NOTE(hi-igpo): tool layer swapped to this repo's tools (web_search + browse_webpage),
# so IGPO's tools_server.MessageClient / SYSTEM_PROMPT imports are dropped. See TOOLS below
...
"""
            ...
            assert False, f"unknown search_engine: {self.config.search_engine}"
    ...
    def execute_predictions(self,
        ...
        """直接调用工具（无需文件 IPC）—— this repo's tool layer (web_search + browse_webpage)."""
    ...
    def parse_response(self, input_ids: torch.Tensor, think: bool = False) -> List[Tuple[bool, str, str]]:
        ...
        This repo's tool layer: format is `[reasoning] <answer>...</answer>` or
        `[reasoning] <tool_call>{json}</tool_call>` (no <think> tags); validates this
        repo's tool arg schema (web_search.query / browse_webpage.url_list).
        """
    ...
    def run_llm_loop(self, gen_batch: DataProto, global_steps: int, ground_truths: list) -> Tuple[Dict, Dict]:
        ...
        for step in range(self.config.max_turns):
            ...
            try:
                ...
                for msg in activate_messages_list:
                    try:
                        result = self.tokenizer.apply_chat_template([msg], add_generation_prompt=True, tools=self.tools, tokenize=False)
        ...
        if activate_list != []:
            for i in activate_list:
                ...
                message_string_list[i] = self.tokenizer.apply_chat_template(messages_list[i], add_generation_prompt=False, tools=self.tools, tokenize=False)
