...
def apply_monkey_patch_to_llama():
    ...
    elif is_transformers_version_in_range("4.48.0", "4.49.0") or is_transformers_version_gte("4.50.0"):
...
def apply_monkey_patch(config: PretrainedConfig, verbose=True):
    if not is_transformers_version_gte("4.45.0"):
        raise AssertionError("The installed `transformers` version doesn't support ulysses patch. "
                             "Please install a version >= 4.45.0 to use this ulysses feature.")
...
@lru_cache()
def is_transformers_version_gte(min_version: str) -> bool:
    try:
        transformers_version = importlib.metadata.version("transformers")
    except importlib.metadata.PackageNotFoundError:
        raise ModuleNotFoundError("The `transformers` package is not installed.")
    return version.parse(transformers_version) >= version.parse(min_version)
